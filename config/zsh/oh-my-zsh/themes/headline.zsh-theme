...
#HEADLINE_PROMPT="%(#..%(!.!!!.%F{200}%F{15}%F{14}%f)) " # consider "%#"
HEADLINE_PROMPT="%(#..%(!.!!!.%B%F{magenta}%F{15}%F{blue}%f)) " # consider "%#"

# Exit status
HEADLINE_CMD_STATUS='true'
HEADLINE_CMD_SUCCESS_STYLE=$bold$green
eval HEADLINE_STYLE_CMD_SUCCESS="\$reset\$HEADLINE_CMD_SUCCESS_STYLE"
HEADLINE_CMD_SUCCESS_CHAR="%{$HEADLINE_STYLE_CMD_SUCCESS%} ✔ "
HEADLINE_CMD_FAIL_STYLE=$bold$red
eval HEADLINE_STYLE_CMD_FAIL="\$reset\$HEADLINE_CMD_FAIL_STYLE"
HEADLINE_CMD_FAIL_CHAR="%{$HEADLINE_STYLE_CMD_FAIL%} ✘ "
HEADLINE_CMD_PROMPT="%(?.${HEADLINE_CMD_SUCCESS_CHAR}.${HEADLINE_CMD_FAIL_CHAR})"
...
# Joints
HEADLINE_USER_BEGIN=''
HEADLINE_USER_TO_HOST=' on '
HEADLINE_HOST_TO_PATH=' in '
HEADLINE_PATH_TO_BRANCH=' working on ' # only used when no padding between <path> and <branch>
...
HEADLINE_STYLE_JOINT=$light_white
if [ "${UID}" = 0 ]; then
...
HEADLINE_STYLE_HOST=$bold$blue
HEADLINE_STYLE_PATH=$bold$magenta
HEADLINE_STYLE_BRANCH=$bold$blue
...
# Prompt
headline_output() {
  local HEADLINE_PRE_PROMPT=
  if [ $HEADLINE_CMD_STATUS = 'true' ]; then HEADLINE_PRE_PROMPT+="${HEADLINE_CMD_PROMPT}"; fi
  HEADLINE_PRE_PROMPT+="${HEADLINE_PROMPT}"
  print -rP $_HEADLINE_INFO_OUTPUT
  print -rP $HEADLINE_PRE_PROMPT
}
if [[ $HEADLINE_INFO_MODE == 'precmd' ]]; then
  local HEADLINE_PRE_PROMPT=
  if [ $HEADLINE_CMD_STATUS = 'true' ]; then
    eval _reset="\$reset"
    HEADLINE_CMD_PROMPT="$HEADLINE_CMD_PROMPT%{$_reset%}"
    HEADLINE_PRE_PROMPT+="${HEADLINE_CMD_PROMPT}%{$_reset%}"
  fi
  HEADLINE_PRE_PROMPT+="${HEADLINE_PROMPT}"
  PROMPT=$HEADLINE_PRE_PROMPT # line and info printed by precmd
else
